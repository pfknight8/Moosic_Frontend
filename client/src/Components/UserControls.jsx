...
export default function Edit({user, LogOut}) {
  ...
  const deleteAccount = async () => {
    const res = await Client.delete(`${BASE_URL}/api/user/${user.id}`, { data: {
      id: user.id,
      password: oldPassword
    }})
  ...
  return (
    <div className='editInfo'>
      <div id='usernameUpdateForm'>
        ...
        <button className='buttonz' id='editUsernameBtn' onClick={handleEditBtn}>{editUsername ? 'Cancel' : 'Edit'}</button>
      </div>
      <div id='emailUpdateForm'>
        ...
        </form>) : <h4>Current email address: {user.email}</h4> }
        <button className='buttonz' id='editEmailBtn' onClick={handleEditBtn}>{editEmail ? 'Cancel' : 'Edit'}</button>
      </div>
      <div id='passwordUpdateForm'>
        ...
        <button className='buttonz' id='editPasswordBtn' onClick={handleEditBtn}>{editPassword ? 'Cancel' : 'Edit'}</button>
      </div>
        ...
        {wantDelete ? (
          <div id='deleteProtect'>
            ...
            <input
              ...
              required
            />
            <button className='buttonZ' disabled={!oldPassword} onClick={deleteAccount}>Yes, Delete Account</button>
          </div>) : null}
          <button className='buttonZ' onClick={() => toggleWantDelete(!wantDelete)}>{wantDelete ? 'No, Keep my Account!' : 'Delete Account'}</button>
      </div>
    </div>
  )
}
